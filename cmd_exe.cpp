...
/*
 * cmd_exec - execute command, if result is not NULL, store
 ...
 */
int cmd_exec(const char *cmd, char *result, int res_len)
{
	int ret_val = FAILED;
	int client_sockfd = 0;
	...
	if (client_sockfd == -1) {
		print_error_ex("fail to create TCP socket.\n");
		return ret_val;
	}
	print_info_ex("cmd: %s\n", cmd);
	...
	if (cmdbuf_len > CMD_MAX_BUF_LEN) {
		print_warning_ex("cmd len is greate than %d\n",
	...
	if (header.flag == CMD_F_NEED_RESULT) {
		...
		result[recvsz] = '\0';
		print_info("cmd: %s\n", cmd);
		my_print_info("result len=%d, context:\n\t%s\n", recvsz, result);
	}
	ret_val = SUCCESS;
exit:
	if (client_sockfd > 0)
